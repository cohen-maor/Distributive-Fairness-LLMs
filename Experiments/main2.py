...
type_to_model = {
    ...
    'llama': ['llama-3.2-90b-text-preview', 'llama3-70b-8192'],
    'chatgpt': ["gpt-4o"],
    'claude': ['claude-3-5-sonnet-20240620']
}
for llm_type in ['llama']:
    ...
    for model in type_to_model[llm_type][:1]:
        ...
        for question in [2,3,5,6,7]:
            if not os.path.exists(f'results/{llm_type}/{model_file}/question_{question}'):
                ...
                filename = f'results/{llm_type}/{model_file}/question_{question}/word.csv'
                pg = PromptGenerator(question=question, valuation_type='word', llm_type=llm_type)
